...
def dct_bloque_2d(b):
    """DCT 2D (tipo II) por multiplicación matricial."""
    return C8 @ b @ C8.T


def idct_bloque_2d(D):
    """IDCT 2D (tipo III) inverso ortogonal."""
    return C8.T @ D @ C8
...
def pad_a_multiplo(img, N=8):
    """Rellena la imagen para que ambos ejes sean múltiplos de N."""
    h, w = img.shape
    nh = ((h + N - 1) // N) * N
    nw = ((w + N - 1) // N) * N
...
def dct_compresion(img, q_factor=0.5):
    ...
    Acepta imágenes en [0,1] (float) o en [0,255] (uint8/float).
    Devuelve: reconstruida, psnr, img_padded, reconstruida_padded
    """
    # --- Normalizar siempre a float32 en [0,1] ---
    img_p = np.array(img, dtype=np.float32)

    max_val = img_p.max()
    if max_val > 1.5:      # si viene en 0–255, escalar
        img_p /= 255.0

    # DEPURACIÓN: ver rango de entrada
    print(f"[DCT] img_p min={img_p.min():.4f}, max={img_p.max():.4f}")
    ...
    psnr = calcular_psnr(img_p[:h, :w], recon)
...
def mostrar_dct(img, q_factor=0.5, guardar=None):
    ...
    # DEPURACIÓN: ver rango de la reconstrucción
    print(f"[DCT] rec min={rec.min():.4f}, max={rec.max():.4f}")
    ...
    # NORMALIZAR PARA QUE SE VEA EN PANTALLA
    rec_show = rec.copy()
    rec_show = np.clip(rec_show, 0, 1)
    rec_show = (rec_show * 255).astype(np.uint8)

    plt.imshow(rec_show, cmap='gray')
...
def main():
    ...
    parser.add_argument('--filtro', type=str, default='butterworth',
                        choices=['ideal', 'gaussiano', 'butterworth'], help='Tipo de filtro en frecuencia')
    parser.add_argument('--tipo', type=str, default='lowpass',
                        choices=['lowpass', 'highpass'], help='Tipo de filtro: pasa bajas o pasa altas')
    ...
    mostrar_fft(img, filtro=args.filtro, tipo=args.tipo, cutoff=args.cutoff,
                orden=args.orden, guardar=os.path.join(args.salidas, 'fft_filtrado.png'))
