...
class TabPractica6(tk.Frame):
    ...
    def _input_for_filter(self):
        ...
        si hay noisy, se filtra la ruidosa; si no, la gris.
        """
    ...
    def load_image(self):
        ...
        h, w = img.shape[:2]
        self.lbl_info.config(text=f"{path}\n{w}×{h}")
        self.viewer.configure(image="", text="Usa el botón 'Convertir a grises' para procesar la imagen")
        self.state.set_from_bgr(path, img)
    ...
    def apply_salt_pepper(self):
        if not self._ensure_gray():
            return
        self.noisy = p6.add_salt_pepper(self.gray, amount=float(self.sp_amount.get()))
        self.filtered = None
    ...
    def apply_salt(self):
        if not self._ensure_gray():
            return
        self.noisy = p6.add_salt(self.gray, amount=float(self.sp_amount.get()))
        self.filtered = None
        self._show(self.noisy)
    ...
    def apply_pepper(self):
        if not self._ensure_gray():
            return
        self.noisy = p6.add_pepper(self.gray, amount=float(self.sp_amount.get()))
        self.filtered = None
        self._show(self.noisy)
    ...
    def apply_gaussian_noise(self):
        if not self._ensure_gray():
            return
        self.noisy = p6.add_gaussian_noise(self.gray, sigma=int(self.gauss_sigma.get()))
        self.filtered = None
        self._show(self.noisy)
...
class TabPractica7(tk.Frame):
    """
    Usa el código de la profesora (practica_frecuencia_ISC.py) a través
